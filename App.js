...
class HomeScreen extends React.Component {
    ...
    constructor(props) {
        ...
        this._resetNodes = helpers._resetNodes.bind(this);

        // Bindings -- NFC.
        this._nfcStart = nfc._nfcStart.bind(this);
        this._nfcNdefRegistrationCheck = nfc._nfcNdefRegistrationCheck.bind(this);
        ...
        // Bindings -- Blockchain.
        this._loadContracts = blockchain._loadContracts.bind(this);
        this._prefetchChainData = blockchain._prefetchChainData.bind(this);
        this._fetchChainData = blockchain._fetchChainData.bind(this);
        this._getBridgeData = blockchain._getBridgeData.bind(this);

        // Bindings -- Verification.
        this._verifyEscrow = verification._verifyEscrow.bind(this);
        this._verifyMerkleProof = verification._verifyMerkleProof.bind(this);
        ...
        this._verifyInternalSignature = verification._verifyInternalSignature.bind(this);
    ...
    _getInitialState = () => {
        ...
        return {
            ...
            _launchStatus: strings.textLaunchOpen,
            _getStartedButtonText: strings.textButtonLoading,
            _getStartedButtonStyle: buttonStyles.buttonLoading,
            _getStartedButtonDisabled: false,       // We may wish to block in certain conditions
    ...
    _resetContracts = () => {
        this._loadContracts().then(() => {
            this._prefetchChainData('getDeviceProofs').then(() => {
                console.log(`reset device proofs complete.`)
                this.setState({_resetButtonText: strings.textSettingsReset});
            }, (error) => {
                this.setState({_resetButtonText: strings.textSettingsResetFail});
            });            
        });
    };

    _resetToDefaultSettings = () => {
        this.setState({_resetButtonText: strings.textSettingsResetPending});

        this._resetNodes().then(() => {
            this._resetContracts()
        });
    };

    _refreshDeviceProofs = async() => {
        // Compare the stored device root count to the server number to see if we need to retrieve.
        let latestDeviceRootCount = await this._prefetchChainData('getDeviceRootCount');

        // textButtonLoading -- should be default
        MMKV.getInt("deviceRootCount", (err, storedDeviceRootCount) => {
            if (storedDeviceRootCount < 1 || storedDeviceRootCount != latestDeviceRootCount) {
                // TODO: block user? show indication that roots are loading?
                console.log(`proof count ${storedDeviceRootCount} does not match ${latestDeviceRootCount}, reload.`);
                this._prefetchChainData('getDeviceProofs').then(() => {
                    console.log(`swap button info after getting proofs.`)
                    this.setState({
                        _launchStatus: null,
                        _getStartedButtonText: strings.textButtonGetStarted,
                        _getStartedButtonStyle: buttonStyles.buttonGetStarted,
                        _getStartedButtonDisabled: false
                    });
                }, (error) => {
    ...
    componentDidMount() {
        ...
        // Detect first launch.
        MMKV.getString("previouslyLaunched", (error, value) => {

            //this._goToScreen(!value ? 'atFirstLaunch' : 'atHome');
            this._goToScreen('atFirstLaunch');
            if (!value) {MMKV.setString('previouslyLaunched', 'true')}
                ...
                { (this.state.location == 'atFirstLaunch') &&
                    ...
                    <View style={viewStyles.viewFirstLaunchVideo}>
                        ...
                        <Video
                            ...
                            source={require('./assets/kongdrop.mp4')}
                        ...
                        <View style={viewStyles.viewFirstLaunchOverlay}>
                            ...
                            <React.Fragment>
                                ...
                                <Text style={textStyles.textLaunchStatus}>{this.state._launchStatus}</Text>

                                <Button
                                    title={this.state._getStartedButtonText}
                                    titleStyle={textStyles.textButtonGetStarted}
                                    buttonStyle={this.state._getStartedButtonStyle}
                                    onPress={() => this._goToScreen('atHome', '')}
                                    disabled={this.state._getStartedButtonDisabled}
                                />
                        ...
                        <View style={viewStyles.viewHomeTop}>
                            ...
                            <View style={viewStyles.viewHomeTopImage}>
                                ...
                                { (Platform.OS == 'ios') &&
                                    ...
                                    <Image source={require('./assets/img/iphone-hands.png')} resizeMode='center' style={imageStyles.imageScan, { width: "100%", height: "100%" }} />

                                }

                                { (Platform.OS == 'android') &&

                                    <Image source={require('./assets/img/android-hands.png')} resizeMode='center'/>
                            ...
                            <View style={viewStyles.viewSettingsBody}>
                                ...
                                <React.Fragment>
                                    ...
                                    */}
                                    ...
                                    <Text style={textStyles.textSettingsScan}>Ethereum</Text>
                                    <TextInput
                                        style={textStyles.textSettingsNodeIpValue}
                                        onChangeText={(text) => {

                                            var chainSettings = Object.assign({}, this.state.chainSettings, {ethNode: text});
                                            this.setState({chainSettings}, () => {MMKV.setString('ethNode', text, (res) => {})})

                                        }}
                                        multiline={true}
                                        value={`${this.state.chainSettings.ethNode}`}
                                    />

                                    <Text style={textStyles.textSettingsScan}>IPFS</Text>
                                    <TextInput
                                        style={textStyles.textSettingsNodeIpValue}
                                        onChangeText={(text) => {

                                            var chainSettings = Object.assign({}, this.state.chainSettings, {ipfsNode: text});
                                            this.setState({chainSettings}, () => {MMKV.setString('ipfsNode', text, (res) => {})})

                                        }}
                                        multiline={true}
                                        value={`${this.state.chainSettings.ipfsNode}`}
                                    />                                    

                                    <Text style={textStyles.textSettingsReset} onPress={() => this._resetToDefaultSettings()} key={this.state._resetButtonText}>{this.state._resetButtonText}</Text>
                            ...
                            <View style={viewStyles.viewFAQDescription}>
                                ...
                                <React.Fragment>
                                    ...
                                    <View style={viewStyles.viewFAQBorder}/>
                                        ...
                                        <Text style={textStyles.textFAQQuestion}>{strings.textFAQQuestion6}</Text>
                                        <Text style={textStyles.textFAQAnswer}>{strings.textFAQAnswer6}</Text>

                                    <View style={viewStyles.viewFAQBorder}/>

                                        <Text style={textStyles.textFAQQuestion}>{strings.textFAQQuestion7}</Text>
                                        <Text style={textStyles.textFAQAnswer}>{strings.textFAQAnswer7}</Text>                                        
